...
from pointnet2_ops_lib.pointnet2_ops.pointnet2_modules import PointnetFPModule, PointnetSAModule, FeatureMapModule
from pointnet2_ops_lib.pointnet2_ops.pointnet2_utils import QueryAndGroup
...
from model.models.denoised.pointnet2_ssg_sem import PointNet2SemSegSSG, calc_t_emb, swish
from model.models.denoised.pnet import Pnet2Stage
from model.models.denoised.model_utils import get_embedder
...
class PointNet2CloudCondition(PointNet2SemSegSSG):
    ...
    def _build_model(self):
        ...
        in_fea_dim = self.hparams['in_fea_dim'] # 48         
        partial_in_fea_dim = self.hparams.get('partial_in_fea_dim', in_fea_dim) # 48 
        ...
        self.use_position_encoding = self.hparams.get('use_position_encoding', False) # fasle
        ...
        self.partial_in_fea_dim = partial_in_fea_dim  # 51 

        self.include_abs_coordinate = self.hparams['include_abs_coordinate'] #True
        self.pooling = self.hparams.get('pooling', 'max') # max 
        ...
        if self.concate_partial_with_noisy_input:   # false 
        ...
        self.fc_t1 = nn.Linear(t_dim, 2*t_dim)
        self.fc_t2 = nn.Linear(2*t_dim, 2*t_dim)
        ...
        if self.include_local_feature:
            ...
            #pdb.set_trace()
            print(" build local condition SA model ")
            self.SA_modules_condition = self.build_SA_model(npoint_condition, radius_condition, 
                                    nsample_condition, feature_dim_condition, mlp_depth_condition, 
                                    partial_in_fea_dim, False, False,
            ...
            print( " build local condtion SA module done ")
            # attention transformer condition  
            ...
            #print(" build FeatureMapModule ")
            #pdb.set_trace()
            for i in range(len(encoder_feature_map_dim)):
                #print("   i  encoder feature mapper  :" , i)
                if i==0:
                    first_conv = self.hparams["bn_first"]  # bn_first  is ture  input  is output of condtion SA module
                    if first_conv:
            ...
            print(" build FeatureMapModule  done ")
        ...
        # SA model
        print( " build SA model for encoder noised ") 
        #pdb.set_trace()
        ...
        print( " build  SA model for encoder noised done ")
        if self.include_local_feature:
            # build FP module for condition cloud
            print(" build include local feature fp module ")
            ...
            self.FP_modules_condition = self.build_FP_model(decoder_feature_dim_condition, decoder_mlp_depth_condition, 
                                        ...
                                        activation=self.network_activation, bn=self.bn,
                                        attention_setting=self.attention_setting)
            print(" build include local feature fp module done ")
    ...
    def forward(self, pointcloud, condition=None, ts=None, label=None, use_retained_condition_feature=False):
        ...
        #  if concate_partial(condition points) with noisy input points   
        #  false 
        ...
        with torch.no_grad():
            ...
            # true 
            #! condition process and points cloud process 
            #pdb.set_trace()
            if self.attach_position_to_input_feature:  
            ...
            #! pointcloud = [ xyz , features ]  
            #! condition  = [ uvw , cond_features]
        ...
        if (not label is None) and self.hparams['include_class_condition']:
            ...
            class_emb = None
        #! if use global_feature 
        #! 是否使用条件
        if self.include_global_feature: # global feature 
        ...
        # condition points xyz l_uvw feature l_cond_features 
        if not condition is None:
            l_uvw, l_cond_features = [uvw], [cond_features]

        #! input points xyz l_xyz l_features 
        l_xyz, l_features = [xyz], [features]  #  [xyz]  b n 3    features  b n 1 
        #pdb.set_trace()
        for i in range(len(self.SA_modules)):
            # false 
            #print(" SA_modules i : " , i )
            if self.include_local_feature:
                ...
                if (use_retained_condition_feature and not self.encoder_cond_features is None):
                    ...
                    #in ours case shoud apply 
                    #pdb.set_trace()
                    # i 0  l_uvw 1 4096 3  l_cond_features 1 4096 128
                    #
                    ...
                    #  l_uvw  cond_feature  l_xyz  pooling  transformer for query conditon feature max approxi l_features 
                    mapped_feature = self.encoder_feature_map[i](l_uvw[i], l_cond_features[i], l_xyz[i], subset=False, 
                                    record_neighbor_stats=self.record_neighbor_stats, pooling=self.pooling,
                                    features_at_new_xyz = l_features[i])
                    #print(" mapped_feature out out : " , mapped_feature.shape)
                #  mapped_feature from condition feature  , l_features from  xyz feature  mapped c1 +  1 
                input_feature = torch.cat([ mapped_feature, l_features[i] ], dim=1)  
            else:
                input_feature = l_features[i]
            #pdb.set_trace()
            # sa_modules : input l_xyz , input_feature ,  t_emb  condition_emb 
            li_xyz, li_features = self.SA_modules[i](l_xyz[i], input_feature, t_emb=t_emb, 
                                    ...
                                    subset=True, record_neighbor_stats=self.record_neighbor_stats,
                                    pooling=self.pooling)
            # each down sample coord  and features 
            # get multi level li_xyz  li_features 
            #pdb.set_trace()
            l_xyz.append(li_xyz)
            l_features.append(li_features)  #  1 , 
        # false 
        #
        #pdb.set_trace()
        if self.include_local_feature:
        ...
        for i in range(-1, -(len(self.FP_modules) + 1), -1):
            ...
            if self.include_local_feature:
                if (use_retained_condition_feature and not self.decoder_cond_features is None):
                    ...
                    mapped_feature = self.decoder_feature_map[i](l_uvw[i], l_cond_features[i], l_xyz[i],
                                                subset=False, record_neighbor_stats=self.record_neighbor_stats, 
                                                pooling=self.pooling, features_at_new_xyz = l_features[i]) 
                    #print("mapped_feature output  fp   " , mapped_feature.shape)
            ...
            l_features[i - 1] = self.FP_modules[i](
                ...
                record_neighbor_stats=self.record_neighbor_stats, pooling=self.pooling)
            #print(" decoder fp modules out put : " , l_features[i - 1].shape)
        ...
        if self.include_local_feature:
            ...
            if (use_retained_condition_feature and not self.decoder_cond_features is None):
                ...
                mapped_feature = self.decoder_feature_map[0](l_uvw[0], l_cond_features[0], l_xyz[0],
                                        subset=False, record_neighbor_stats=self.record_neighbor_stats, pooling=self.pooling,
                                        features_at_new_xyz = l_features[0])
                #print(" output last mapped feature " , mapped_feature.shape)
