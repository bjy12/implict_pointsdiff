...
class DGCNN_cls(nn.Module):
    ...
    def forward(self, p):
        ...
        #pdb.set_trace()
        c = self.dgcnn_encoder(p)  # 1 , N , 128 
        #pdb.set_trace()
        fea = {}
        if 'grid' in self.plane_type:
            fea['grid'] = self.generate_grid_features(coord, c)
        if 'xz' in self.plane_type:
            fea['xz'] = self.generate_plane_features(coord, c, plane='xz')
        if 'xy' in self.plane_type:
            fea['xy'] = self.generate_plane_features(coord, c, plane='xy')
        if 'yz' in self.plane_type:
            fea['yz'] = self.generate_plane_features(coord, c, plane='yz')
        #pdb.set_trace()
        return fea
