...
from model.modules.PV_module.struct_feat_transformer.factory import build_dropout_layer

from model.modules.PV_module.struct_feat_transformer.output_layer import AttentionOutput
...
class RPEMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_q, input_k, input_v, embed_qk, key_weights=None, key_masks=None, attention_factors=None):
        ...
        """
        #pdb.set_trace()
        " b n (h c)  2 4096 256  h = 4 ->  2 4 4096 64  "
        " b m (h c)  2 4096 256  h = 4 ->  2 4 4096 64  "
        ' b m (h c)  2 4096 256  h = 4 ->  2 4 4096 64  '
        ' '
        ...
        p = rearrange(self.proj_p(embed_qk), 'b n (h c) -> b h n c', h=self.num_heads)  # 修改这里

        #attention_scores_p = torch.einsum('bhnc,bhnmc->bhnm', q, p)
        attention_scores_p = torch.einsum('bhnc,bhmc->bhnm', q, p)  # 修改einsum表达式
...
class RPETransformerLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, activation_fn='ReLU'):
        ...
        self.linear_input = nn.Linear(2*d_model, d_model)
        self.linear_memory = nn.Linear(2*d_model , d_model)
    ...
    def forward(
        ...
        input_states = self.linear_input(input_states)
        memory_states = self.linear_memory(memory_states)
        hidden_states, attention_scores = self.attention(
            input_states,
            memory_states,
            position_states,
