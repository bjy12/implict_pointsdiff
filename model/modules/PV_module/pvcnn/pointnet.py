...
from model.modules.PV_module.pvcnn.ball_query import BallQuery , BallQuery_Temb
from model.modules.PV_module.pvcnn.shared_mlp import SharedMLP
import pdb
__all__ = ['PointNetAModule', 'PointNetSAModule', 'PointNetFPModule' ,'PointNetSA_TEmb_LocalModule' , 'PointNetFP_TembModule']
...
class PointNetSA_TEmb_LocalModule(nn.Module):
    def __init__(self, num_centers, radius, num_neighbors, in_channels, out_channels, include_coordinates=True):
        super().__init__()
        if not isinstance(radius, (list, tuple)):
            radius = [radius]
        ...
        for _radius, _out_channels, _num_neighbors in zip(radius, out_channels, num_neighbors):
            groupers.append(
                BallQuery_Temb(radius=_radius, num_neighbors=_num_neighbors, include_coordinates=include_coordinates)
            )
            #pdb.set_trace()
            mlps.append(
                SharedMLP(in_channels=in_channels + (3 if include_coordinates else 0),
    ...
    def forward(self, inputs):
        #pdb.set_trace()
        features, global_coords, temb , local_coord = inputs
        # use local coords for voxelization 
        centers_coords , centers_coords_local = F.furthest_point_sample(global_coords, self.num_centers , local_coord,return_indices=True)
        features_list = []
        for grouper, mlp in zip(self.groupers, self.mlps):
            #pdb.set_trace()
            features, temb = mlp(grouper(global_coords, centers_coords, temb, features))
            features_list.append(features.max(dim=-1).values)
        if len(features_list) > 1:
            return features_list[0], centers_coords, temb.max(dim=-1).values , centers_coords_local if temb.shape[1] > 0 else temb
        else:
            return features_list[0], centers_coords, temb.max(dim=-1).values , centers_coords_local if temb.shape[1] > 0 else temb
...
class PointNetFP_TembModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.mlp = SharedMLP(in_channels=in_channels, out_channels=out_channels, dim=1)
    ...
    def forward(self, inputs):
        ...
        if len(inputs) == 3:
            ...
            points_coords, centers_coords, centers_features, points_features, temb , local_coord = inputs
        interpolated_features = F.nearest_neighbor_interpolate(points_coords, centers_coords, centers_features)
        ...
        return self.mlp(interpolated_features), points_coords, interpolated_temb , local_coord