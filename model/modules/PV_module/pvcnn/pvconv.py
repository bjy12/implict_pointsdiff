...
from model.modules.PV_module.pvcnn.se import SE3d ,SE3d_Relu
import pdb
...
class PVConv_Attention(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, resolution, attention=False,
        ...
        if with_se:
            voxel_layers.append(SE3d_Relu(out_channels, use_relu=with_se_relu))
        self.voxel_layers = nn.Sequential(*voxel_layers)
        self.point_features = SharedMLP(in_channels, out_channels)
...
class PVConv_AttLocal(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, resolution, attention=False,
                 dropout=0.1, with_se=False, with_se_relu=False, normalize=True, eps=0 , return_local_coord=True):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.resolution = resolution
        self.return_local_coord = return_local_coord
    ...
    def forward(self, inputs):
        #* local_coord for voxelization 
        #pdb.set_trace()
        features, coords, temb , local_coord = inputs
        voxel_features, voxel_coords = self.voxelization(features, local_coord)
        voxel_features = self.voxel_layers(voxel_features)
        voxel_features = F.trilinear_devoxelize(voxel_features, voxel_coords, self.resolution, self.training)
        fused_features = voxel_features + self.point_features(features)
        if self.return_local_coord:
            return fused_features, coords, temb , local_coord
        else:
            return fused_features, coords, temb 
